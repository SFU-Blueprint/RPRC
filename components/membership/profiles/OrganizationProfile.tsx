...
import { robotoCondensed, bodyStyles, buttonStyles, subheaderStyles } from '@/app/fonts';
import type { OrganizationMemberDashboardData } from '@/types/membership.types';

type Props = {
    data: OrganizationMemberDashboardData | null;
    onEdit?: () => void;
};

export default function OrganizationProfile({ data, onEdit }: Props) {
    if (!data) return null;
    ...
    return (
        <div>
            ...
            <hr className="my-4 border-application-detail-border-50" />
            <h3 className="text-base md:text-lg lg:text-xl font-medium p-3">Representative Information</h3>
            <div className="mb-2">
                <div className="flex p-2 items-center gap-2">
                    <User className="text-primary" />
                    <p className={bodyStyles.lg}>{data?.representativeName ?? ''}</p>
                </div>
                {data?.representativeEmail && (
                    <div className="flex p-2 items-center gap-2">
                        <Mail className="text-primary" />
                        <p className={bodyStyles.lg}>{data.representativeEmail}</p>
                    </div>
                )}
            </div>
            <hr className="my-4 border-application-detail-border-50" />
            <div>
                ...
                <h3 className={`${subheaderStyles.s} text-content-secondary p-3 mt-6`}>What services does your organization offer ?</h3>
                <p className={`${bodyStyles.lg} text-content-primary rounded-2xl border border-1 border-application-detail-border-50 bg-card-background-gray p-5`}>
